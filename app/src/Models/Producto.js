var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
import Utilerias from "./Utilerias";
var Producto = /** @class */ (function () {
    function Producto() {
        this.imagenes = [];
        this.imagenesURL = [];
    }
    Producto.prototype.addImagenes = function (imgs) {
        return __awaiter(this, void 0, void 0, function () {
            var resultado, i, imagen, ERROR_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        resultado = [];
                        i = 0;
                        _a.label = 1;
                    case 1:
                        if (!(i < imgs.length)) return [3 /*break*/, 7];
                        imagen = {
                            codigoERROR: 0,
                            mensajeERROR: "",
                            base64: "",
                            tipoImagen: ""
                        };
                        _a.label = 2;
                    case 2:
                        _a.trys.push([2, 4, , 5]);
                        return [4 /*yield*/, Utilerias.getImagen(imgs[i], ["jpg", "png", "jpeg"], 5000000)];
                    case 3:
                        imagen = _a.sent();
                        return [3 /*break*/, 5];
                    case 4:
                        ERROR_1 = _a.sent();
                        console.log(ERROR_1);
                        resultado.push(Producto.ERROR_NO_IMAGEN);
                        return [3 /*break*/, 6];
                    case 5:
                        switch (imagen.codigoERROR) {
                            case Utilerias.ERROR_IMAGEN_TIPO:
                            case Utilerias.ERROR_IMAGEN_LECTURA:
                                resultado.push(Producto.ERROR_NO_IMAGEN);
                                return [3 /*break*/, 6];
                            case Utilerias.ERROR_IMAGEN_SIZE_MAX:
                                resultado.push(Producto.ERROR_TAMAÑO_MAX);
                                return [3 /*break*/, 6];
                        }
                        //________________________
                        // VERIFICAR SI IMAGENES == 4
                        //________________________
                        if (this.imagenes.length == 4) {
                            resultado.push(Producto.ERROR_LIMITE_MAX);
                            return [3 /*break*/, 6];
                        }
                        //________________________
                        // TODO BIEN
                        //________________________
                        this.imagenesURL.push(imagen.base64);
                        resultado.push(Producto.EXITOSO);
                        this.imagenes.push(imgs[i]);
                        _a.label = 6;
                    case 6:
                        i++;
                        return [3 /*break*/, 1];
                    case 7: return [2 /*return*/, resultado];
                }
            });
        });
    };
    Producto.prototype.eliminarImagen = function (index) {
        this.imagenes.splice(index, 1);
        var FilesEliminados = this.imagenesURL.splice(index, 1);
        if (FilesEliminados.length != 0)
            return true;
        return false;
    };
    Producto.prototype.clonar = function () {
        var nuevoProducto = new Producto();
        nuevoProducto.imagenes = this.imagenes.slice(0, this.imagenes.length);
        nuevoProducto.imagenesURL = this.imagenesURL.slice(0, this.imagenesURL.length);
        return nuevoProducto;
    };
    Producto.EXITOSO = 0;
    Producto.ERROR_LIMITE_MAX = 1;
    Producto.ERROR_NO_IMAGEN = 2;
    Producto.ERROR_TAMAÑO_MAX = 3;
    return Producto;
}());
export default Producto;
